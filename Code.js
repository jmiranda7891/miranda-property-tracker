...
// `propietario` is shown to users as "Dueño Beneficiario" and `propEscriturado` as "Nombre
// Escriturado" - the column keys stay as-is (they already held the right underlying data),
// only the UI labels changed, in Code.js v2 schema (see CLAUDE.md "Schema v2"). `propuestaTraspaso`
...
// is 4 real-world options instead of 9, and planLargoPlazo (3 options) replaced the old
// free-text propuestaTraspaso succession field. This runs unconditionally from every
// ensureRegistry_() call (both branches), guarded by SCHEMA_V2_DONE for a cheap no-op after
// the first pass, but is ALSO idempotent per-row-per-field (only rewrites a field if its
// current value isn't already a valid new-shape value) - that's the real safety net, since a
// fresh install seeds directly in the new shape and a global flag alone can't tell "already
// migrated" apart from "never needed migrating" without risking a double-remap. See
// CLAUDE.md "Schema v2 migration" for the exact mapping rules and why they're best-effort.
var STATUS_LEGACY_MAP_ = { in_use: 'en_uso', for_sale: 'en_venta', sold: 'vendida' };
var STATUS_LEGACY_NOTE_ = {
  in_process: 'Proceso', ejido: 'Ejido', family_matter: 'Asunto familiar',
  investment: 'Inversión', held_by_company: 'A nombre de empresa'
};
function migrateSchemaV2_(ss) {
  if (prop_('SCHEMA_V2_DONE') === 'true') return;
  var sh = ss.getSheetByName('Properties');
  if (!sh) return;
  ['esEjido', 'planLargoPlazo', 'lat', 'lng'].forEach(function (c) { ensureColumn_(sh, c); });

  var latlngByRef = {};
  SEED_PROPERTIES_.forEach(function (p) { latlngByRef[p.referencia] = { lat: p.lat, lng: p.lng }; });
  ...
  for (var r = 1; r < vals.length; r++) {
    ...
    var rawEscrituras = String(row[idx.escrituras] || '').trim();
    var isEjido = /ejido/i.test(rawEscrituras);
    if (rawEscrituras !== 'Si' && rawEscrituras !== 'No') {
      var newEscrituras = /^s/i.test(rawEscrituras) ? 'Si' : 'No';
      if (rawEscrituras && !isEjido && newEscrituras === 'No') notes.push('Escrituras (dato original): ' + rawEscrituras);
      sh.getRange(r + 1, idx.escrituras + 1).setValue(newEscrituras);
    }
    var esEjidoCell = row[idx.esEjido];
    if (esEjidoCell !== true && esEjidoCell !== false && esEjidoCell !== 'TRUE' && esEjidoCell !== 'FALSE') {
      sh.getRange(r + 1, idx.esEjido + 1).setValue(isEjido);
    }

    var rawStatus = String(row[idx.status] || '').trim();
    if (STATUSES_.indexOf(rawStatus) < 0) {
      var newStatus = STATUS_LEGACY_MAP_[rawStatus] || 'libre';
      if (!STATUS_LEGACY_MAP_[rawStatus] && STATUS_LEGACY_NOTE_[rawStatus]) notes.push('Estatus anterior: ' + STATUS_LEGACY_NOTE_[rawStatus]);
      sh.getRange(r + 1, idx.status + 1).setValue(newStatus);
    }

    var rawPlan = String(row[idx.planLargoPlazo] || '').trim();
    if (PLANES_LARGO_PLAZO_.indexOf(rawPlan) < 0) {
      var rawTraspaso = String(row[idx.propuestaTraspaso] || '').trim();
      var newPlan = /vender/i.test(rawTraspaso) ? 'vender' : (/fideicomiso/i.test(rawTraspaso) ? 'fideicomiso' : 'mantener_individual');
      if (rawTraspaso) notes.push('Traspaso (dato original): ' + rawTraspaso);
      sh.getRange(r + 1, idx.planLargoPlazo + 1).setValue(newPlan);
    }

    var latCell = row[idx.lat];
    if (latCell === '' || latCell == null) {
      var ll = latlngByRef[String(row[idx.referencia] || '')];
      if (ll) {
        sh.getRange(r + 1, idx.lat + 1).setValue(ll.lat);
        sh.getRange(r + 1, idx.lng + 1).setValue(ll.lng);
      }
    }

    if (notes.length) {
      var existingNotes = String(row[idx.observacionesRaw] || '');
      sh.getRange(r + 1, idx.observacionesRaw + 1).setValue((existingNotes ? existingNotes + ' | ' : '') + notes.join(' | '));
    }
  }
  setProp_('SCHEMA_V2_DONE', 'true');
}
...
// ============================ Properties: read ============================
// Schema v2 (see CLAUDE.md "Schema v2 migration"): status is exactly these 4 real-world
// options now, not the old 9-value list. escrituras is Si/No only - ejido tenure is its own
// `esEjido` flag, independent of deed status. `planLargoPlazo` replaced the old free-text
// `propuestaTraspaso` succession field with exactly 3 options.
var STATUSES_ = ['en_uso', 'libre', 'en_venta', 'vendida'];
var TIPOS_ = ['Casa', 'Departamento', 'Edificio', 'Local', 'Oficina', 'Terreno', 'Otro'];
var PLANES_LARGO_PLAZO_ = ['mantener_individual', 'fideicomiso', 'vender'];
...
function addProperty(form) {
  ...
  var status = STATUSES_.indexOf(form.status) >= 0 ? form.status : 'libre';
  var escrituras = form.escrituras === 'Si' ? 'Si' : 'No';
  var planLargoPlazo = PLANES_LARGO_PLAZO_.indexOf(form.planLargoPlazo) >= 0 ? form.planLargoPlazo : 'mantener_individual';
  var id = uid_();
  var me = getUserEmail();
  var now = nowIso_();
  var row = {
...
var PLAN_EXPORT_LABEL_ = {
  mantener_individual: 'Keep Individual / Mantener Individual',
  fideicomiso: 'Transfer to Trust / Traspasar a Fideicomiso',
  vender: 'Sell / Vender'
};
...
function exportProperties(filterJson) {
  ...
  var out = selected.map(function (p) {
    return EXPORT_HEADERS_.map(function (h) {
      ...
      if (k === 'esEjido') return p.esEjido ? 'Yes / Sí' : 'No';
      if (k === 'planLargoPlazo') return PLAN_EXPORT_LABEL_[p.planLargoPlazo] || p.planLargoPlazo;
      return p[k] == null ? '' : p[k];
    });
  });
...
var SEED_PROPERTIES_ = [
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Casa",
    "referencia": "Casa Maravatio",
    "propietario": "Jorge Miranda Juárez",
    "direccion": "Leona Vicario #110",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "En Uso | Traspaso (dato original): Fideicomiso (Compartido)",
    "status": "en_uso",
    "precioEstimadoUSD": 1400000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "México",
    "tipo": "Edificio",
    "referencia": "Edificio IESA",
    "propietario": "Inorporated Express, Pamela Miranda Molina",
    "direccion": "Ocampo 51 Col. Centro Maravatío, Mich.",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "En Uso | Traspaso (dato original): Fideicomiso (Renta)",
    "status": "en_uso",
    "precioEstimadoUSD": 1370000,
    "escrituras": "Si",
    "propEscriturado": "Incorporated Express, S.A de C.V",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Edificio",
    "referencia": "Oficina OE Maravatio",
    "propietario": "Jorge Miranda Juarez",
    "direccion": "Madero #55",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "a nombre de CC | Estatus anterior: A nombre de empresa | Traspaso (dato original): Fideicomiso (Renta)",
    "status": "libre",
    "precioEstimadoUSD": null,
    "precioEstimadoIsPlaceholder": true,
    "escrituras": "Si",
    "propEscriturado": "OECC",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Local",
    "referencia": "Oficina Cd. Hidalgo",
    "propietario": "Casa de Cambio",
    "direccion": "Cuauhtemoc Sur 6",
    "ciudad": "Cd. Hidalgo",
    "estado": "Michoacan",
    "cp": "61100",
    "observacionesRaw": "En Uso | Traspaso (dato original): JORGE A",
    "status": "en_uso",
    "precioEstimadoUSD": 1370000,
    "escrituras": "Si",
    "propEscriturado": "OECC",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Casa",
    "referencia": "Casa Guanajuato",
    "propietario": "Georgina M. Lopez",
    "direccion": "Tenaza #28, Col. San Javier",
    "ciudad": "Guanajuato",
    "estado": "Guanajuato",
    "cp": "",
    "observacionesRaw": "En Uso | Traspaso (dato original): JORGE A",
    "status": "en_uso",
    "precioEstimadoUSD": 350000,
    "escrituras": "Si",
    "propEscriturado": "Georgina Molina López",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Local",
    "referencia": "Buzon Purpecha",
    "propietario": "Jorge Miranda Juarez",
    "direccion": "Leona Vicario #235, esq. con Josefa Ortiz de Dominguez",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "En Uso | Traspaso (dato original): JORGE A",
    "status": "en_uso",
    "precioEstimadoUSD": 230000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Terreno",
    "referencia": "Cantalagua",
    "propietario": "Jorge A. Miranda",
    "direccion": "Lote #11, Zona D. Fraccionamiento y Club de Golf Hacienda Cantalagua",
    "ciudad": "Contepec",
    "estado": "Michoacan",
    "cp": "",
    "observacionesRaw": "Traspaso (dato original): JORGE A",
    "status": "libre",
    "precioEstimadoUSD": 137000,
    "escrituras": "Si",
    "propEscriturado": "Jorge A. Miranda",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Terreno",
    "referencia": "Terreno Autopista",
    "propietario": "Jorge A. Miranda",
    "direccion": "Tramo Federal Maravatio-Atlacomulco, Ejido la Huaracha (Juan Francisco)",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "Traspaso (dato original): JORGE A",
    "status": "libre",
    "precioEstimadoUSD": 80000,
    "escrituras": "No",
    "propEscriturado": "Jorge A. Miranda",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Local",
    "referencia": "Oficina Acambaro",
    "propietario": "Jorge Miranda Juárez, Jorge A. Miranda",
    "direccion": "Miguel Hidalgo 335-C",
    "ciudad": "Acambaro",
    "estado": "Guanajuato",
    "cp": "38600",
    "observacionesRaw": "En Uso | Traspaso (dato original): PAMELA",
    "status": "en_uso",
    "precioEstimadoUSD": 300000,
    "escrituras": "Si",
    "propEscriturado": "Jorge A. Miranda",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Casa",
    "referencia": "Carmen Lopez",
    "propietario": "Jorge Miranda Juarez",
    "direccion": "Leona Vicario 895 con Trece de Septiembre",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "En Uso | Traspaso (dato original): PAMELA",
    "status": "en_uso",
    "precioEstimadoUSD": 200000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Local",
    "referencia": "Local Acambaro 2-A",
    "propietario": "Georgina M. Lopez",
    "direccion": "Zaragoza 2-A",
    "ciudad": "Acambaro",
    "estado": "Guanajuato",
    "cp": "38600",
    "observacionesRaw": "En Uso | Traspaso (dato original): PAMELA",
    "status": "en_uso",
    "precioEstimadoUSD": 120000,
    "escrituras": "Si",
    "propEscriturado": "Georgina Molina López",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Local",
    "referencia": "Local Acambaro 2-B",
    "propietario": "Georgina M. Lopez",
    "direccion": "Zaragoza 2-B",
    "ciudad": "Acambaro",
    "estado": "Guanajuato",
    "cp": "38600",
    "observacionesRaw": "En Uso | Traspaso (dato original): PAMELA",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Local",
    "referencia": "Oficina Salvatierra",
    "propietario": "Jorge Miranda Juárez",
    "direccion": "Juarez 215-A",
    "ciudad": "Salvatierra",
    "estado": "Guanajuato",
    "cp": "38900",
    "observacionesRaw": "En Uso | Traspaso (dato original): PAMELA",
    "status": "en_uso",
    "precioEstimadoUSD": 90000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "propietario": "Pamela Miranda Molina",
    "direccion": "Sta Rosa camino a Palomas",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "En Uso | Traspaso (dato original): PAMELA",
    "status": "en_uso",
    "precioEstimadoUSD": 34000,
    "escrituras": "No",
    "propEscriturado": "",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Terreno",
    "referencia": "Yurecuaro",
    "propietario": "Jorge Miranda Juarez",
    "direccion": "Yurecuaro",
    "ciudad": "Yurecuaro",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "VENDER | Traspaso (dato original): PAMELA",
    "status": "en_venta",
    "precioEstimadoUSD": 30000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Terreno",
    "referencia": "El Tejero",
    "propietario": "Jorge Miranda Juarez",
    "direccion": "El Tejero",
    "ciudad": "Tejero",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "VENDER",
    "status": "en_venta",
    "precioEstimadoUSD": 30000,
    "escrituras": "No",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Edificio",
    "referencia": "La Victoria",
    "propietario": "Jorge Miranda Juarez",
    "direccion": "Prol. Galeana #1657, Fracc. San Antonio",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "Asunto JMJ | Estatus anterior: Asunto familiar",
    "status": "libre",
    "precioEstimadoUSD": 450000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Terreno",
    "referencia": "Panuco, Zac",
    "propietario": "Jorge Miranda Juarez",
    "direccion": "Zacatecas, Zac.",
    "ciudad": "Panuco",
    "estado": "Zacatecas",
    "cp": "",
    "observacionesRaw": "VENDER",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Terreno",
    "referencia": "Funera Express",
    "propietario": "Pamela Miranda Molina",
    "direccion": "Ejido Tzinzingareo, Municipio de Irimbo",
    "ciudad": "Tzinzingareo",
    "estado": "Michoacan",
    "cp": "",
    "observacionesRaw": "Traspaso (dato original): PAMELA",
    "status": "libre",
    "precioEstimadoUSD": null,
    "escrituras": "No",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Terreno",
    "referencia": "Terreno Manzanillo",
    "propietario": "Jorge, Francisco, Toño",
    "direccion": "Manzanillo, Col.",
    "ciudad": "Manzanillo",
    "estado": "Colima",
    "cp": "",
    "observacionesRaw": "VENDER | Escrituras (dato original): Pendientes | Traspaso (dato original): VENDER",
    "status": "en_venta",
    "precioEstimadoUSD": 950000,
    "escrituras": "No",
    "propEscriturado": "",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Terreno",
    "referencia": "Predio Acambaro",
    "propietario": "Consorcio Inmobiliario JMJ",
    "direccion": "Predio Rustico CERRO DE LA CRUZ",
    "ciudad": "Acambaro",
    "estado": "Guanajuato",
    "cp": "38600",
    "observacionesRaw": "VENDER",
    "status": "en_venta",
    "precioEstimadoUSD": 364000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "",
    "tipo": "Terreno",
    "referencia": "Rancho Grande",
    "propietario": "Jorge Miranda Juárez",
    "direccion": "Salida Salvatierra",
    "ciudad": "Acambaro",
    "estado": "Guanajuato",
    "cp": "",
    "observacionesRaw": "",
    ...
    "precioEstimadoUSD": 200000,
    "escrituras": "No",
    "propEscriturado": "en tramite Escritura",
  ...
  },
  {
    ...
    "referencia": "Jungapeo",
    "propietario": "Jorge Miranda Juárez",
    "direccion": "La Garita, Lazaro Cardenas",
    "ciudad": "Jungapeo",
    "estado": "Michoacan",
    "cp": "",
    "observacionesRaw": "VENDER | Escrituras (dato original): Proceso",
    "status": "en_venta",
    "precioEstimadoUSD": 180000,
    "escrituras": "No",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Casa",
    "referencia": "INFONAVIT",
    "propietario": "Jorge Miranda Juárez",
    "direccion": "And. Onesimo Lopez Couto #86, Fracc. Francisco J. Mujica",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "VENDER",
    "status": "en_venta",
    "precioEstimadoUSD": 23000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Casa",
    "referencia": "San Nicolas",
    "propietario": "Jorge Miranda Juárez",
    "direccion": "Salazar 204 Col. Centro",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "",
  ...
  },
  {
    ...
    "referencia": "Puebla",
    "propietario": "Jorge Miranda Juarez",
    "direccion": "Calle 5 Sur, Col. el Humilladero",
    "ciudad": "Tehuacan",
    "estado": "Puebla",
    "cp": "",
    "observacionesRaw": "VENDER",
    "status": "en_venta",
    "precioEstimadoUSD": 14000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "Mexico",
    "tipo": "Casa",
    "referencia": "Francisco I Madero",
    "propietario": "Jorge Miranda Juarez",
    "direccion": "Venustiano Carranza 429, Frac. Francisco I. Madero",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "VENDER",
    "status": "en_venta",
    "precioEstimadoUSD": 12000,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "",
    "tipo": "Terreno",
    "referencia": "Inversion Qro.",
    "propietario": "Centro Comercial Tecnologico 998 S.A de C.V",
    "direccion": "Lot. 6, Manzana ZRUM 05, Fracc. Zaru",
    "ciudad": "Queretaro",
    "estado": "Queretaro",
    "cp": "",
    "observacionesRaw": "Inversion | Estatus anterior: Inversión",
    "status": "libre",
    "precioEstimadoUSD": 45000,
    "escrituras": "No",
    "propEscriturado": "",
  ...
  },
  {
    ...
    "referencia": "max",
    "propietario": "Jorge Miranda Juárez",
    "direccion": "Comonfort 13-B Centro",
    "ciudad": "Maravatio",
    "estado": "Michoacan",
    "cp": "61250",
    "observacionesRaw": "En Uso",
    "status": "en_uso",
    "precioEstimadoUSD": null,
    "escrituras": "Si",
    "propEscriturado": "Jorge Miranda Juarez",
  ...
  },
  {
    ...
    "paisRaw": "",
    "tipo": "Terreno",
    "referencia": "Torreón Monte Bello",
    "propietario": "Jorge Miranda Juárez",
    "direccion": "Torreon",
    "ciudad": "Torreon",
    "estado": "Coahuila",
  ...
  {
    "countryCode": "US",
    "paisRaw": "Estados Unidos",
    "tipo": "Terreno",
    "referencia": "Estacionamiento P443",
    "propietario": "J&G Properties",
    "direccion": "501 N. Clinton St., Parking #P443",
    "ciudad": "Chicago",
    "estado": "Illinois",
    "cp": "60654",
    "observacionesRaw": "STATE PLANNING",
    "status": "libre",
    "precioEstimadoUSD": 30000,
    "pin": "17-09-112-107-1439",
  ...
  },
  {
    ...
    "paisRaw": "Estados Unidos",
    "tipo": "Edificio",
    "referencia": "Edificio J.A",
    "propietario": "Jorge A. Miranda",
    "direccion": "669 W. Ohio St",
    "ciudad": "Chicago",
    "estado": "Illinois",
    "cp": "60654",
    "observacionesRaw": "JORGE A",
    "status": "libre",
    "precioEstimadoUSD": 800000,
    "pin": "17-09-103-002-0000",
  ...
  },
  {
    ...
    "propietario": "J&G Services",
    "direccion": "685 W. Ohio St.",
    "ciudad": "Chicago",
    "estado": "Illinois",
    "cp": "60654",
    "observacionesRaw": "STATE PLANNING",
    "status": "libre",
    "precioEstimadoUSD": 2000000,
  ...
  },
  {
    ...
    "paisRaw": "Estados Unidos",
    "tipo": "Edificio",
    "referencia": "Edificio Oficina Aurora",
    "propietario": "J&G Properties",
    "direccion": "22 N Union St",
    "ciudad": "Aurora",
    "estado": "Illinois",
    "cp": "60505",
    "observacionesRaw": "STATE PLANNING (source data listed the city as Chicago; corrected to Aurora, IL to match the address)",
    "status": "libre",
  ...
  },
  {
    ...
    "paisRaw": "Estados Unidos",
    "tipo": "Casa",
    "referencia": "Casa Willow",
    "propietario": "Alemap, LLC",
    "direccion": "(sold — street address not recorded in source data)",
    "ciudad": "Chicago",
    "estado": "Illinois",
    "cp": "60605",
    "observacionesRaw": "Marked \"vendida\" (sold) in the original source data",
    "status": "vendida",
    "precioEstimadoUSD": 750000,
    "archived": true,
    "archivedReason": "Sold ('vendida' in source data) — archived at migration",
  ...
  },
  {
    ...
    "paisRaw": "Estados Unidos",
    "tipo": "Terreno",
    "referencia": "Estacionamiento P52",
    "propietario": "J&G Properties",
    "direccion": "501 N. Clinton St., Parking #P52",
    "ciudad": "Chicago",
    "estado": "Illinois",
    "cp": "60654",
    "observacionesRaw": "STATE PLANNING",
